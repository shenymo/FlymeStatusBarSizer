...
final class LauncherRecentsTouchController {
    ...
    private static boolean runStackClearAllDismissAnimation(
            ...
            Class<?> taskViewClass,
            long durationMs) {
        LauncherRecentsPerf.beginSession("clearAll", recentsView);
        Object pendingAnimation = LauncherRecentsCompat.createPendingAnimationInstance(
                pendingAnimationConstructor,
                durationMs);
        if (pendingAnimation == null) {
            LauncherRecentsPerf.finishSession("clearAll", recentsView, "abort");
            return false;
        }
    ...
    private static Boolean handleStackDismissControllerTouch(
        ...
        if (!shouldHandleStackDismissTouch(recentsView)) {
    ...
    private static boolean shouldKeepStackDismissGestureAwayFromPagedView(
        ...
        if (recentsView == null
                || motionEvent == null
                || motionEvent.getActionMasked() != MotionEvent.ACTION_MOVE
                || !shouldHandleStackDismissTouch(recentsView)
    ...
    private static void beginStackDismissDrag(
        ...
        LauncherRecentsPerf.beginSession("dismissTask", state.recentsView);
        state.dragging = true;
    ...
    private static void beginStackMenuDrag(
        ...
        LauncherRecentsTaskVisuals.setTranslationZ(
                state.taskView,
                state.originalTranslationZ);
        prepareStackMenuReveal(state);
        applyStackDismissProgress(state, state.currentDismissTranslation);
    }

    private static void prepareStackMenuReveal(StackDismissGestureState state) {
        int taskIndex = findTaskViewIndex(state.recentsView, state.taskView);
        if (taskIndex <= 0) {
            return;
        }
        View coverTaskView = LauncherRecentsCompat.getTaskViewAt(
                state.recentsView,
                taskIndex - 1);
        if (!isStackDismissTaskCandidate(state.recentsView, coverTaskView)) {
            return;
        }
        boolean primaryHorizontal = isPrimaryScrollHorizontal(state.recentsView);
        float coverStart = resolveStackTaskPrimaryStart(
                state.recentsView,
                coverTaskView,
                primaryHorizontal);
        float coverEnd = resolveStackTaskPrimaryEnd(
                state.recentsView,
                coverTaskView,
                primaryHorizontal);
        float taskStart = resolveStackTaskPrimaryStart(
                state.recentsView,
                state.taskView,
                primaryHorizontal);
        float taskEnd = resolveStackTaskPrimaryEnd(
                state.recentsView,
                state.taskView,
                primaryHorizontal);
        if (coverEnd <= taskStart || coverStart >= taskEnd) {
            return;
        }
        float coverCenter = (coverStart + coverEnd) * 0.5f;
        float taskCenter = (taskStart + taskEnd) * 0.5f;
        float gap = FlymeStatusBarSizer.dp(state.recentsView.getContext(), 12);
        float shift = coverCenter <= taskCenter
                ? taskStart - coverEnd - gap
                : taskEnd - coverStart + gap;
        state.menuCoverPrimaryHorizontal = primaryHorizontal;
        state.menuCoverShift = shift;
        state.menuCoverStartOffsets = new HashMap<>();
        for (int i = 0; i < taskIndex; i++) {
            ...
            state.menuCoverStartOffsets.put(
                    taskView,
    ...
    private static float resolveStackTaskPrimaryStart(
            View recentsView,
            View taskView,
            boolean primaryHorizontal) {
        float scale = Math.max(
                0.01f,
                primaryHorizontal ? Math.abs(taskView.getScaleX()) : Math.abs(taskView.getScaleY()));
        float pivot = primaryHorizontal ? taskView.getPivotX() : taskView.getPivotY();
        float position = primaryHorizontal ? taskView.getX() : taskView.getY();
        int scroll = primaryHorizontal ? recentsView.getScrollX() : recentsView.getScrollY();
        return position - scroll + pivot - (pivot * scale);
    }

    private static float resolveStackTaskPrimaryEnd(
            View recentsView,
            View taskView,
            boolean primaryHorizontal) {
        float size = primaryHorizontal ? taskView.getWidth() : taskView.getHeight();
        float scale = Math.max(
                0.01f,
                primaryHorizontal ? Math.abs(taskView.getScaleX()) : Math.abs(taskView.getScaleY()));
        return resolveStackTaskPrimaryStart(recentsView, taskView, primaryHorizontal)
                + (size * scale);
    }
    ...
    private static void settleAppToRecentsForStackDismiss(View recentsView) {
        ...
        LauncherRecentsTransitionController.normalizeAppToRecentsStackAnchor(recentsView);
        LauncherRecentsState.setAppToRecentsStackSettled(recentsView, true);
    ...
    private static void settleOverviewStateForStackDismiss(View recentsView) {
        if (recentsView == null
                || !LauncherRecentsStateAnimationController.isOverviewStateStackAnimationActive(
                recentsView)) {
            return;
        }
        logStackFlow("dismiss:overviewEntryTakeover", recentsView, null, null);
        LauncherRecentsTransitionController.forceRecentsTranslationZero(recentsView);
        LauncherRecentsStateAnimationController.finishOverviewStateStackAnimationForTouchTakeover(
                recentsView);
        LauncherRecentsState.setLastStackLayoutApply(recentsView, null);
    ...
    private static void updateStackMenuDrag(
        ...
        state.pendingMenuRevealProgress = LauncherRecentsLayoutEngine.clamp(
                distance / Math.max(1f, threshold),
                0f,
                1f);
        scheduleStackDismissDragFrame(state);
    }
    ...
    private static void finishStackDismissGesture(
        ...
        if (state.menuDragging) {
            animateStackMenuReturn(state, !canceled && state.menuThresholdReached);
    ...
    private static void animateStackMenuReturn(
        ...
        float startRevealProgress = state.menuRevealProgress;
    ...
    private static void animateStackDismissCancel(StackDismissGestureState state) {
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                resetStackDismissVisuals(state);
                LauncherRecentsPerf.finishSession("dismissTask", state.recentsView, "cancel");
            }
        });
        animator.start();
    }
    ...
    private static void applyPendingStackDismissDragFrame(StackDismissGestureState state) {
        ...
        applyStackDismissProgress(state, state.currentDismissTranslation);
        applyStackMenuRevealProgress(state, state.pendingMenuRevealProgress);
        applyStackDismissRelayoutProgress(state, state.pendingRelayoutProgress);
    }

    private static void applyStackMenuRevealProgress(
            StackDismissGestureState state,
            float progress) {
        if (state == null
                || state.menuCoverStartOffsets == null
                || state.menuCoverStartOffsets.isEmpty()) {
            return;
        }
        float clampedProgress = LauncherRecentsLayoutEngine.clamp(progress, 0f, 1f);
        state.menuRevealProgress = clampedProgress;
        for (View taskView : state.menuCoverStartOffsets.keySet()) {
            float startOffset = state.menuCoverStartOffsets.get(taskView);
            float offset = startOffset + (state.menuCoverShift * clampedProgress);
            if (state.menuCoverPrimaryHorizontal) {
                LauncherRecentsTaskVisuals.setTaskOffsetTranslationX(taskView, offset);
            } else {
                LauncherRecentsTaskVisuals.setTaskOffsetTranslationY(taskView, offset);
            }
        }
        state.recentsView.invalidate();
    }
    ...
    private static void preloadStackDismissFillTask(
            StackDismissGestureState state,
            HashMap<View, StackDismissRelayoutStartState> startStates,
            HashMap<View, LauncherRecentsTaskVisuals.StackTaskVisualState> targetStates) {
        for (View taskView : targetStates.keySet()) {
            ...
            if (startState == null
                    || targetState == null
                    || Math.min(startState.visualState.attachAlpha,
                    startState.visualState.stableAlpha) > 0.01f
                    || Math.min(targetState.attachAlpha, targetState.stableAlpha) <= 0.01f) {
                continue;
            }
            LauncherRecentsCompat.invokeCompat(
                    taskView,
                    ...
                    true,
                    3);
            state.preloadedFillTaskView = taskView;
            return;
        }
    }

    private static void clearStackDismissFillTaskPreload(StackDismissGestureState state) {
        if (state == null || state.preloadedFillTaskView == null) {
            return;
        }
        if (!isTaskDataTracked(state.recentsView, state.preloadedFillTaskView)) {
            LauncherRecentsCompat.invokeCompat(
                    state.preloadedFillTaskView,
                    "onTaskListVisibilityChanged",
                    LauncherRecentsCompat.BOOLEAN_INT_ARG,
                    false,
                    3);
        }
        state.preloadedFillTaskView = null;
    }

    private static boolean isTaskDataTracked(View recentsView, View taskView) {
        ...
        if (!(trackedData instanceof SparseBooleanArray) || !(taskIds instanceof int[])) {
            return false;
        }
        for (int taskId : (int[]) taskIds) {
            if (taskId >= 0 && ((SparseBooleanArray) trackedData).get(taskId)) {
                return true;
            }
        }
        return false;
    ...
    private static View findRemainingTaskView(View recentsView, View dismissedTaskView) {
        int taskViewCount = LauncherRecentsCompat.invokeInt(recentsView, "getTaskViewCount", 0);
        for (int i = 0; i < taskViewCount; i++) {
            View taskView = LauncherRecentsCompat.getTaskViewAt(recentsView, i);
            if (taskView != null
    ...
    private static void resetStackDismissVisuals(StackDismissGestureState state) {
        ...
        applyStackMenuRevealProgress(state, 0f);
    ...
    private static void clearStackDismissGesture(View recentsView, boolean resetVisuals) {
        ...
        if (state.dragging) {
            LauncherRecentsPerf.finishSession("dismissTask", recentsView, "abort");
        }
    ...
    private static boolean commitStackDismiss(StackDismissGestureState state) {
        ...
        int taskViewCount = LauncherRecentsCompat.invokeInt(recentsView, "getTaskViewCount", 0);
        View singleRemainingTaskView = taskViewCount == 2
                ? findRemainingTaskView(recentsView, taskView)
                : null;
        if (state.relayoutPrepared) {
            ...
            if (singleRemainingTaskView != null) {
    ...
    private static void finishSingleTaskDismissHandoff(
            View recentsView,
        ...
        ViewTreeObserver.OnPreDrawListener listener = new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                ...
                applySingleTaskDismissHandoff(recentsView, remainingTaskView);
                return true;
            }
        };
        recentsView.getViewTreeObserver().addOnPreDrawListener(listener);
        recentsView.requestLayout();
    }
    ...
    private static void applySingleTaskDismissHandoff(
        ...
        if (recentsView == null
                ...
                || LauncherRecentsCompat.invokeInt(recentsView, "getTaskViewCount", 0) != 1
                || LauncherRecentsCompat.getTaskViewAt(recentsView, 0) != remainingTaskView) {
            return;
        }
        LauncherRecentsCompat.invokeCompat(
                recentsView,
        ...
        remainingTaskView.setVisibility(View.VISIBLE);
        LauncherRecentsLayoutEngine.restoreTaskTransform(remainingTaskView);
        setStackDismissTranslationX(remainingTaskView, 0f);
        setStackDismissTranslationY(remainingTaskView, 0f);
        LauncherRecentsTaskVisuals.setAttachAlpha(remainingTaskView, 1f);
        LauncherRecentsTaskVisuals.setStableAlpha(remainingTaskView, 1f);
        LauncherRecentsTaskVisuals.forceRecentsTaskHeadsVisible(recentsView);
        LauncherRecentsTaskVisuals.clearAppliedTaskState(remainingTaskView);
        LauncherRecentsTaskVisuals.captureCurrentTaskStatesAsBaseline(recentsView);
    ...
    private static final class StackDismissGestureState {
        ...
        HashMap<View, Float> menuCoverStartOffsets;
        boolean dragging;
        boolean menuDragging;
        boolean menuThresholdReached;
        boolean menuCoverPrimaryHorizontal;
        ...
        float menuCoverShift;
        float menuRevealProgress;
        float pendingMenuRevealProgress;
