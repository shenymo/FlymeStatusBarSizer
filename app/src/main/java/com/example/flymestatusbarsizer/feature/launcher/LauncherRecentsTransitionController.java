...
final class LauncherRecentsTransitionController {
                ...
                if (thisObject instanceof View) {
                    ...
                    if (shouldAnimateBlankTapHomeExit(recentsView)) {
                        ...
                        try {
                            ...
                            LauncherRecentsPerf.end("native:startHome", recentsView, nativeStartNs);
                        }
                    }
            ...
            module.intercept(method, chain -> {
                ...
                try {
                    ...
                    LauncherRecentsPerf.end(
                            "native:onPrepareGestureEndAnimation",
                            recentsView,
                            nativeStartNs);
                }
                if (recentsView != null
    ...
    static void prepareBlankTapHomeExitAnimation(View recentsView) {
        ...
        LauncherRecentsPerf.beginSession("returnHome", recentsView);
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (cancelled) {
                    ...
                    LauncherRecentsState.clearPositionOwner(
                            recentsView,
                            LauncherRecentsState.POSITION_OWNER_ENTER);
                    LauncherRecentsPerf.finishSession("enterRecents", recentsView, "cancel");
                    return;
                }
                if (!handoffStarted[0]) {
                    handoffStartScroll[0] = resolvePrimaryScroll(recentsView);
    ...
    private static int resolveAppToRecentsStackAnchorPage(View recentsView) {
        ...
        if (runningTaskObject instanceof View && recentsView instanceof ViewGroup) {
            ...
            if (runningTaskPage >= 0) {
                if (LauncherRecentsLayoutEngine.currentAppCenteredOnEntry(recentsView)) {
                    return Math.min(runningTaskPage, pageCount - 1);
                }
